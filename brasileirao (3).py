# Nomes:                           ADS 3º A
# Cleriston Junior Ra: 2201072
# Guilherme Rocha  Ra: 2201325
# Mateus Pereira   RA: 2201058
# João Pedro Paulino Fonseca RA: 2201068
...
def nome_campeao(dados):
    numero_campeao = id_campeao(dados)
    return nome_do_time(dados,numero_campeao)
...
Ela retorna o numero de times que o brasileirao qualifica para a libertadores
...
def qtos_libertadores(dados):
    classificados = []
    if 'Libertadores' in dados['fases']['2700']['faixas-classificacao']['classifica1']['texto']:
        qtd = (dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa'])
        classificados = qtd.split('-')
        return int(classificados[1])
...
def ids_dos_melhor_classificados(dados,numero_de_times):
    ...
    return classificacao[0:numero_de_times]        
...
def classificados_libertadores(dados):
    ...
    times_classificados = ids_dos_melhor_classificados(dados, numero_de_times)
...
def nomes_dos_times_de_um_jogo(dados, id_jogo):
    time1_id, time2_id = ids_dos_times_de_um_jogo(dados, id_jogo)
    time1_nome = dados['equipes'][time1_id]['nome-comum']
    time2_nome = dados['equipes'][time2_id]['nome-comum']
    return time1_nome, time2_nome
...
def id_do_time(dados, nome_time):
    for equipe_id, equipe_info in dados['equipes'].items():
        if equipe_info['nome-comum'] == nome_time:
            return equipe_id
...
def datas_de_jogo(dados):
    datas = []
    for data, jogos in dados['fases']['2700']['jogos']['data'].items():
        datas.append(data)
    return datas
...
def busca_imprecisa_por_nome_de_time(dados, nome_time):
    ids_times = []
    for time_id, time in dados['equipes'].items():
        if any(nome_time.lower() in str(time[campo]).lower() for campo in ['nome-comum', 'nome-slug', 'sigla', 'nome']):
...
def ids_de_jogos_de_um_time(dados, time_id):
    jogos_time = []
    for jogo_id, jogo in dados['fases']['2700']['jogos']['id'].items():
        if time_id in (jogo['time1'], jogo['time2']):
            jogos_time.append(jogo_id)
    return jogos_time
...
def datas_de_jogos_de_um_time(dados, nome_time):
    ...
    data_jogos = [dados['fases']['2700']['jogos']['id'][jogo_id]['data']for jogo_id in jogos_time]
    return data_jogos
...
def dicionario_de_gols(dados):
    gols = {}
    for jogo in dados['fases']['2700']['jogos']['id'].values():
        time1, time2 = jogo['time1'], jogo['time2']
        placar1, placar2 = int (jogo['placar1']), int (jogo['placar2'])
...
def time_que_fez_mais_gols(dados):
    ...
    time_mais_gols = max(gols, key=gols.get)
    return time_mais_gols
...
def rebaixados(dados):
    classificacao = dados['fases']['2700']['classificacao']['grupo']['Único']
    zona_rebaixamento = dados['fases']['2700']['faixas-classificacao']['classifica3']['faixa']
    inicio,fim = [int(x) for x in zona_rebaixamento.split('-')]
    return classificacao[inicio - 1:fim]
